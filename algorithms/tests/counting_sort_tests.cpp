...
TEST( counting_sort, maximum_value_at_limit )
{
  ...
  ASSERT_THROW( { counting_sort( input, std::numeric_limits<unsigned>::max() ); }, std::length_error );
}
...
{
  auto input = std::vector<unsigned>{ 16, 96, 33, 20, 6,  83, 64, 71, 64, 55, 8,  14, 19, 36, 25,
                                      90, 70, 6,  1,  76, 46, 51, 93, 61, 19, 49, 13, 92, 77, 69 };
  auto result = counting_sort( input, 100 );
  auto expected = std::vector<unsigned>{ 1,  6,  6,  8,  13, 14, 16, 19, 19, 20, 25, 33, 36, 46, 49,
